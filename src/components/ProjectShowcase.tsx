      ...
      <div className="relative mx-auto max-w-7xl">
          ...
          <AnimatePresence>
            ...
            >
              <Image
                ...
                width={100}
                height={100}
                className="h-auto w-1/2 rounded-lg border border-zinc-300 shadow-lg dark:hidden dark:border-pink-400/50"
              ...
              {images[currentImage].DARK !== undefined && (
                <Image
                  ...
                  width={100}
                  height={100}
                  className="hidden h-auto w-1/2 rounded-lg  shadow-lg dark:inline-block dark:shadow-lg "
                  alt={`project ${currentImage}`}
                />
              )}
            </motion.div>
          ...
          {props.projects.map((proj) => (
            ...
            >
              <div className="flex gap-2">
                <span className="text-3xl font-semibold text-pink-600 transition-colors duration-300 dark:text-pink-400 sm:text-4xl md:text-5xl lg:hidden">
                  {proj.index + 1}.
                </span>
                <span
                  key={proj.title}
                  className="-underline-offset-1 text-3xl font-semibold text-pink-600 underline transition-colors duration-300 dark:text-pink-400 sm:text-4xl md:text-5xl lg:hidden"
                >
                  {proj.title}
                </span>
              </div>
        ...
        >
          <div className="relative max-w-max">
            ...
            <span className="absolute -bottom-1 left-0 h-[2px] w-0 origin-left rounded-lg bg-pink-600 transition-[width] duration-300 group-hover:w-full dark:bg-pink-400"></span>
          </div>
          <div className="h-8 w-8">
            <ArrowTopRight className="rotate-45 text-pink-600 transition-transform duration-300 group-hover:rotate-0 group-hover:scale-[1.1] dark:text-pink-400" />
          </div>
        </Link>
      </div>
    </section>
